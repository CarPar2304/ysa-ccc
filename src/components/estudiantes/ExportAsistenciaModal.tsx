...
interface ClaseRow { id: string; modulo_id: string; titulo: string; orden: number | null }

const COHORTES_DISPONIBLES = [1, 2, 3, 4];
...
export const ExportAsistenciaModal = ({ open, onClose, allowedNiveles }: Props) => {
  ...
  const [cohortesSel, setCohortesSel] = useState<number[]>([1]);
  ...
  useEffect(() => { if (open) { setNiveles(allowedNiveles); setCohortesSel([1]); } }, [open, allowedNiveles]);

  useEffect(() => {
    if (!open) return;
    setLoading(true);
    (async () => {
      const { data: mods } = await supabase.from("modulos").select("id, titulo, nivel, orden").in("nivel", niveles).order("orden");
      const modsArr = (mods || []) as ModuloRow[];
      setModulos(modsArr);
      setModulosSel(new Set(modsArr.map((m) => m.id)));
      const modIds = modsArr.map((m) => m.id);
      if (modIds.length) {
        const { data: cls } = await supabase.from("clases").select("id, modulo_id, titulo, orden").in("modulo_id", modIds).order("orden");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Get cupos for selected niveles (+ cohortes filter when aplica)
      ...
      // Build student list (owner + cofounders)
      ...
      for (const modulo of modulosFiltered) {
        ...
        for (const s of modStudents) {
          ...
          if (!u) continue;
          const attendance = modClases.map((c) => asistio.has(`${s.user_id}:${c.id}`));
          const present = attendance.filter(Boolean).length;
          const pct = modClases.length > 0 ? (present / modClases.length) * 100 : 0;
          totalPct += pct; countStu++;
          const row = ws.addRow([u.nombres || "", u.apellidos || "", u.email || "", s.emp_nombre, s.nivel, s.cohorte, s.tipo, ...attendance.map((a) => a ? "✓" : "✗"), `${pct.toFixed(1)}%`]);
          row.eachCell((cell, colNum) => {
            cell.border = border;
            if (colNum > 7 && colNum < headers.length) {
              cell.alignment = { horizontal: "center" };
              cell.font = { color: { argb: attendance[colNum - 8] ? "FF15803D" : "FFB91C1C" }, bold: true };
            }
          });
        }
